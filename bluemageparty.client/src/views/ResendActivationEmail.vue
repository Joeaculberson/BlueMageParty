...
import { RESEND_ACTIVATION_EMAIL } from '@/constants/api';
...
export default {
    ...
    setup() {
        ...
        const resendActivationEmail = async () => {
            ...
            try {
                const response = await axios.post(RESEND_ACTIVATION_EMAIL, { email: email.value });
                
                // Store email in Pinia (authStore)
                authStore.setEmail(email.value);

                alertType.value = 'success';
                message.value = 'Activation email resent successfully! Code/link will expire in 10 minutes.';
