<template>
  ...
  interface Character {
    ...
    avatar: string;
    missingSpells: Spell[];
    fullName: string; // Make fullName required
  }
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const characters = ref<Character[]>([]); // Declare characters as a ref
      ...
      const recalculateEveryoneNeeds = () => {
        ...
        party.value.everyoneNeeds = allSpells.filter((spell) => {
          const isMissingForEveryone = memberOwnedSpells.every((ownedSpells) =>
            ownedSpells.has(spell.id)
          );
      ...
      const searchCharacters = async () => {
        ...
        try {
          ...
          characters.value = response.data.map((char: Character) => ({
            ...char,
            fullName: `${char.firstName} ${char.lastName}`, // Ensure fullName is set
          }));
        } catch (error) {
      ...
      const clearSearch = () => {
        searchQuery.value = "";
        characters.value = []; // Reset characters ref
      };
      ...
      const updatePartyMembers = (updatedPartyMembers: PartyMember[]) => {
      ...
      return {
        ...
        searchQuery,
        selectedCharacter,
        characters, // Return characters so it's accessible in the template
        filteredCharacters,
        addCharacterToParty,
        clearSearch,
        handleSearchUpdate, // Add this function to handle search updates
        updatePartyMembers,
        recalculateEveryoneNeeds,
        currentUserId,
        message,
