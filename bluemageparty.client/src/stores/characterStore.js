    ...
    actions: {
        async fetchVerifiedCharactersFromDB() {
            ...
            if (!authToken)
                return;
            try {
                const response = await apiClient.get(GET_CHARACTERS_URL, {
        ...
        async removeCharacterFromDB(character) {
            try {
                ...
                const response = await apiClient.delete(`${REMOVE_CHARACTER_URL}/${character.id}`);
        ...
        removeVerifiedCharacter(character) {
            const index = this.verifiedCharacters.findIndex((verifiedCharacter) => verifiedCharacter.id === character.id);
            if (index !== -1) {
